...
class GCN(nn.Module):
    def __init__(self, nfeat, nhid, nclass, dropout, adj, prune_adj, cuda):
        ...
        if self.prune_adj:
          ...
          self.adj_weight = nn.parameter.Parameter(torch.FloatTensor(adj), requires_grad=True)
    ...
    def forward(self, x, adj):
        if self.prune_adj:
          if(self.apply_adj_mask == False):
            adj_weight = torch.mul(self.adj_weight, self.adj_mask)
            adj_temp = torch.mul(adj, adj_weight)
          else:
...
class GCN_4layers(nn.Module):
    def __init__(self, nfeat, nhid, nclass, dropout, adj, prune_adj, cuda):
        ...
        self.gc1 = GraphConvolution(nfeat, 16, adj)
        self.gc2 = GraphConvolution(16, 10, adj)
        self.gc3 = GraphConvolution(10, 5, adj)
        self.gc4 = GraphConvolution(5, nclass, adj)
        self.dropout = dropout
        self.prune_adj = prune_adj
        self.apply_adj_mask = False
        self.is_cuda = cuda
    ...
    def forward(self, x, adj):
        ...
        x = F.relu(self.gc1(x, adj_temp))
        x = F.dropout(x, self.dropout, training=self.training)
        x = F.relu(self.gc2(x, adj_temp))
        x = F.dropout(x, self.dropout, training=self.training)
        x = F.relu(self.gc3(x, adj_temp))
        x = F.dropout(x, self.dropout, training=self.training)
        x = self.gc4(x, adj_temp)
        return F.log_softmax(x, dim=1)
