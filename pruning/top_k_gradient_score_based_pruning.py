...
class TopK_GradientScoreBasedPruning():
  ...
  def compute_mask(self, adj, features, labels, idx_train, idx_val, idx_test):
        ...
        if(self.prune_weight):
          for gc1_weight_param in self.model.gc1.top_k_weight_list:
            self.gc1_weight_grad_sum = torch.add(self.gc1_weight_grad_sum, gc1_weight_param['grad'])
            self.gc1_weight_sum = torch.add(self.gc1_weight_sum, gc1_weight_param['weight'])
          for gc2_weight_param in self.model.gc2.top_k_weight_list:
            self.gc2_weight_grad_sum = torch.add(self.gc2_weight_grad_sum, gc2_weight_param['grad'])
            self.gc2_weight_sum = torch.add(self.gc2_weight_sum, gc2_weight_param['weight'])

          scores['gc1_weight'] = torch.clone(torch.div(self.gc1_weight_grad_sum, self.top_k_weight) * torch.div(self.gc1_weight_sum, self.top_k_weight)).detach().abs_()
          scores['gc2_weight'] = torch.clone(torch.div(self.gc2_weight_grad_sum, self.top_k_weight) * torch.div(self.gc2_weight_sum, self.top_k_weight)).detach().abs_()
        ...
        if(self.prune_bias):
          for gc1_bias_param in self.model.gc1.top_k_bias_list:
            self.gc1_bias_grad_sum = torch.add(self.gc1_bias_grad_sum, gc1_bias_param['grad'])
            self.gc1_bias_sum = torch.add(self.gc1_bias_sum, gc1_bias_param['bias'])
          for gc2_bias_param in self.model.gc2.top_k_bias_list:
            self.gc2_bias_grad_sum = torch.add(self.gc2_bias_grad_sum, gc2_bias_param['grad'])
            self.gc2_bias_sum = torch.add(self.gc2_bias_sum, gc2_bias_param['bias'])
          
          scores['gc1_bias'] = torch.clone(torch.div(self.gc1_bias_grad_sum, self.top_k_bias) * torch.div(self.gc1_bias_sum, self.top_k_bias)).detach().abs_()
          scores['gc2_bias'] = torch.clone(torch.div(self.gc2_bias_grad_sum, self.top_k_bias) * torch.div(self.gc2_bias_sum, self.top_k_bias)).detach().abs_()
