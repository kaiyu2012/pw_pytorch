...
class m2_GradientScoreBasedPruning():
  ...
  def compute_mask(self, adj, features, labels, idx_train, idx_val, idx_test):
        ...
        scores = {}
        if(self.prune_weight):
          self.gc1_weight_grad_sum = torch.add(self.gc1_weight_grad_sum, self.model.gc1.weight.grad.abs_())
          self.gc2_weight_grad_sum = torch.add(self.gc2_weight_grad_sum, self.model.gc2.weight.grad.abs_())
        ...
        if(self.prune_bias):
          self.gc1_bias_grad_sum = torch.add(self.gc1_bias_grad_sum, self.model.gc1.bias.grad.abs_())
          self.gc2_bias_grad_sum = torch.add(self.gc2_bias_grad_sum, self.model.gc2.bias.grad.abs_())
        ...
        if self.prune_adj:
          for adj_param in self.model.top_k_adj_list:
            self.adj_grad_sum = torch.add(self.adj_grad_sum, adj_param['adj_grad'].abs_())
