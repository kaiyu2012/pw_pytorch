...
class GradientScoreBasedPruning():
  ...
  def compute_mask(self, adj, features, labels, idx_train, idx_val, idx_test):
        ...
        if(self.prune_weight):
          ...
          scores['gc1_weight'] = torch.clone(torch.div(self.gc1_weight_grad_sum, self.epoch+1) * self.model.gc1.weight).detach().abs_()
          scores['gc2_weight'] = torch.clone(torch.div(self.gc2_weight_grad_sum, self.epoch+1) * self.model.gc2.weight).detach().abs_()
        ...
        if(self.prune_bias):
          ...
          scores['gc1_bias'] = torch.clone(torch.div(self.gc1_bias_grad_sum, self.epoch+1) * self.model.gc1.bias).detach().abs_()
          scores['gc2_bias'] = torch.clone(torch.div(self.gc2_bias_grad_sum, self.epoch+1) * self.model.gc2.bias).detach().abs_()
          bias_scores = torch.cat([torch.flatten(scores['gc1_bias']), torch.flatten(scores['gc2_bias'])])
        if self.prune_adj:
          self.adj_grad_sum = torch.add(self.adj_grad_sum, self.model.best_adj_grad)
          score_adj_weight = torch.clone(torch.div(self.adj_grad_sum, self.epoch+1) * self.model.best_adj_weight).detach().abs_()
